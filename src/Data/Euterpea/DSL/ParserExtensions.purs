...
many1Nel p = unsafePartial (go p)
  where
    go :: ∀ a1 . Partial => Parser a1 -> Parser (Nel.NonEmptyList a1)
    go p' =
      lToNel <$> many1 p'
...
many1TillNel p end = unsafePartial (go p end)
  where
    go :: ∀ a1 end1. Partial => Parser a1 -> Parser end1 -> Parser (Nel.NonEmptyList a1)
    go p' end' =
...
sepBy1Nel p sep = unsafePartial (go p sep)
  where
    go :: ∀ a1 sep1. Partial => Parser a1 -> Parser sep1 -> Parser (Nel.NonEmptyList a1)
    go p' sep' =
      lToNel <$> sepBy1 p' sep'
